...
J'expliquais récemment à un ami que ChatGPT, dans son essence, n'est « qu'un »
...
langage », soit un ensemble de comportements humains que l'on regroupe, de
manière générale, sous l'appellation « répondre à une question ». Il n'y a pas
d'essence profonde et unique de la « réponse-ité », ni de la « question-ité » :
il y a une multitude de comportements vaguement apparentés, impliquant contexte
et langage, que nous désignons ainsi.

Ce que cela pose comme problème, selon moi, est véritablement un problème de
modélisation a priori : depuis aussi longtemps que l'idée d'IA existe, le modèle
a priori de la compréhension du langage (et par implication, de la capacité à
répondre à des questions) a toujours été le suivant : on reçoit une séquence de
symboles (des mots), puis un mécanisme de traitement se charge de « comprendre »
ces mots, c'est-à-dire de construire une sorte de représentation (interne) de
leur signification, quelle que soit la nature de cette « chose » (un modèle
mental interne, un état, une configuration, peu importe). Ce n'est qu'à partir
de là, une fois cette représentation acquise, que quelque chose peut s'ensuivre
(une réponse, dans le cas où l'on répond à une question), qui pourrait d'abord
être conçue, puis exécutée.

![](llm-question-answer-classic-ai.png)
{.center}

Or il semble que la modélisation avec les grands modèles de langage remette ce
modèle en question. Il semble qu'une compréhension réelle puisse émerger du
simple fait d'apprendre adéquatement à prédire une séquence de mots, sans qu'il
soit nécessaire de passer par cette modélisation « intermédiaire », cette boîte
noire. Et c'est là un changement de paradigme très profond et déstabilisant.
